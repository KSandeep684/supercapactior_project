# ==============================================================================
# FINAL CAPSTONE PROJECT: V30 - DEFINITIVE VERSION WITH ALL TABS & FEATURES + AI
# ==============================================================================
    ...
    if output_format == 'Simple Prediction':
        ...
        if ai_enabled:
            if st.button("Get AI Recommendation"):
                with st.spinner("Gemini AI is analyzing your results..."):
                    prompt = f"""
                    You are an expert in supercapacitor technology.
                    Scenario: Material={plot_material}, Electrolyte={plot_electrolyte}, Device={plot_device}, Current Density={plot_current_density}, Cycles={selected_cycles}.
                    The model predicts Charge Capacity = {charge_pred:.2f} mAh/g; Discharge Capacity = {discharge_pred:.2f} mAh/g.
                    Please provide:
                    1. Professional performance summary in one line.
                    2. Two bullet points with strengths and weaknesses.
                    3. One application recommendation.
            ...
            st.info("AI Recommendation feature not enabled. Please configure your GOOGLE_API_KEY in Streamlit secrets to use this feature.")
        ...
        if start_cycle >= end_cycle:
            ...
            # --- AI Recommendation for Data Table/Graph ---
            st.divider()
            st.subheader("🌟 AI Recommendation")
            if ai_enabled:
                if st.button("Get AI Recommendation"):
                    with st.spinner("Gemini AI is analyzing your results..."):
                        ...
                        prompt = f"""
                        You are an expert in supercapacitor technology.
                        Scenario: Material={plot_material}, Electrolyte={plot_electrolyte}, Device={plot_device}, Current Density={plot_current_density}.
                        The model predicts a degradation from {initial_cap:.2f} mAh/g to {final_cap:.2f} mAh/g over {final_cycles} cycles, retention {retention:.1f}%.
                        Please provide:
                        1. Professional summary in one line.
                        2. Two bullet points with insights/risks.
                        3. One specific application recommendation.
...
# --- TAB 4: The Detailed Comparison Tab ---
with tab4:
    ...
    detailed_comparison_data = {
        'Metric': [
            "Specific Capacitance (F/g)", "Energy Density (Wh/kg)", "Power Density (W/kg)",
            "Cycle Life", "Charge Time", "Energy Storage Mechanism"
        ],
        'Conventional Capacitor': [
            "Very Low (~0.0001)", "< 0.1", "> 10,000", "> 1,000,000", "Milliseconds", "Physical (Electric Field)"
        ],
        "This Project's Supercapacitor": [
            "High (100 - 1,200)", "~27.5", "~1,875", "> 50,000", "Seconds to Minutes", "Physical & Chemical (EDLC + Faradaic)"
        ],
        'Lithium-ion (Li-ion) Battery': [
            "N/A (Not a capacitor)", "~150", "~300", "~1,000", "Hours", "Chemical (Intercalation)"
        ],
        'Sodium-ion (Na-ion) Battery': [
            "N/A (Not a capacitor)", "~120", "~200", "~2,000", "Hours", "Chemical (Intercalation)"
        ]
    }
